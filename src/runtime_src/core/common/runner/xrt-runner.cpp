...
usage()
{
  ...
  std::cout << " [--dir <path>]       Directory with artifacts (default: current dir)\n";
  std::cout << " [--report [<file>]]  Output metrics to file or stdout\n";
  std::cout << "\n";
...
static void
run_replay(const std::string& script, const std::string& dir,
           const std::string& report)
{
  xrt_core::systime st;
  xrt::device device{0};
  xrt_core::replay rply{device, script, dir};
  rply.execute();
